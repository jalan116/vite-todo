<template>
  <div class="min-h-screen flex flex-col justify-center items-center">
    <header class="flex flex-col w-full container">
      ...
      <input
        class="text-2xl py-2 px-4 rounded-xl w-full mt-8 text-gray-800"
        type="text"
    ...
    <main class="container w-full mt-8">
      <section class="space-y-4" v-if="pendingTodos.length > 0">
        ...
        <ul class="space-y-4">
          ...
          :key="todo.id"
           class="bg-white rounded-xl text-2xl py-2 [x-4 font-light text-green-800 text-center
           hover:text-white hover:bg-green-800 cursor-pointer transtion-colors"
      ...
      <section class="space-y-4" v-if="completedTodos.length > 0">
        ...
        <ul class="space-y-4">
          ...
          :key="todo.id"
          class="bg-white rounded-xl text-2xl py-2 [x-4 font-light text-red-800 text-center
           hover:text-white hover:bg-red-800 cursor-pointer transtion-colors"
           @click="changeStatus(todo.id)">
           {{todo.text}}</li>
        </ul>
      </section>
